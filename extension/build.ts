...
async function buildOnce(): Promise<boolean> {
  ...
  if (!css.success) { report('styles', css.logs); return false; }
  ...
  if (!content.success) { report('content', content.logs); return false; }
  ...
  if (!bg.success) { report('background', bg.logs); return false; }
  ...
  if (!panel.success) { report('panel', panel.logs); return false; }
