...
export function stakeFor(edge: number, unit = DEFAULT_UNIT): StakeAdvice
{
  ...
  if (edge > 0.35)
    return {
      units: 1,
      sum: unit,
      note: 'подозрительно высокий Edge — перепроверить состав',
      flag: '🟡',
    };
  if (edge >= 0.25)
    return { units: 2, sum: unit * 2, note: 'очень высокая уверенность', flag: '🟢' };
  ...
  return {
    units: 0.5,
    sum: unit * 0.5,
    note: 'монетка с наклоном — минимальный размер',
    flag: '🟡',
  };
}
...
interface RawAnchor
{
  ...
  max: number | null;
  gem: string | null;
  descTeam: string | null;
  descSign: string | null;
  start: number;
  end: number;
  odds?: number;
}
...
function classify(a: RawAnchor): Offer | null
{
  ...
  switch (p[0])
  {
    case 'win':
      ...
      if (p.length === 3)
        return { ...base, market: 'map_win', map: Number(p[1]), team: slot(p[2]) };
      return null;
    case 'pistol':
      return { ...base, market: 'pistol', pistol: Number(p[1]), team: slot(p[2]) };
    case 'first5':
      return { ...base, market: 'first_n', map: Number(p[1]), team: slot(p[2]), n: line };
    case 'roundstot':
      return {
        ...base,
        market: 'rounds_total',
        map: Number(p[1]),
        side: p[2] === '1' ? 'over' : 'under',
        line,
      };
    case 'oddeven':
      return { ...base, market: 'oddeven', map: Number(p[1]), side: p[2] === '1' ? 'even' : 'odd' };
    case 'rounddif':
      return {
        ...base,
        market: 'round_fora',
    ...
    case 'overtime':
      return { ...base, market: 'overtime', map: Number(p[1]) };
    case 'fora':
      return { ...base, market: 'maps_fora', team: slot(p[1]), hcap: signed(-(line ?? 0)) };
    case 'fora2':
      return { ...base, market: 'maps_fora', team: slot(p[1]), hcap: signed(line ?? 0) };
    case 'maps':
...
export function mergeSides(a: Match, b: Match | null | undefined): Match
{
  ...
  for (const s of ['1', '2'] as const)
    if (!teams[s]?.name && b.teams[s]?.name) teams[s] = b.teams[s];
...
export function pairs(m: Match): MarketPair[]
{
  ...
  push(
    `Тотал карт ${mt[0]?.line ?? 2.5}`,
    'maps_total',
    mt.find((o) => o.side === 'over'),
    mt.find((o) => o.side === 'under'),
  );
  ...
  push(
    'Чёт / нечёт',
    'oddeven',
    oe.find((o) => o.side === 'even'),
    oe.find((o) => o.side === 'odd'),
  );
  ...
  for (const line of [...new Set(mf.map((o) => Math.abs(o.hcap ?? 0)))].sort((x, y) => x - y))
  {
    for (const t of ['1', '2'] as const)
    {
      ...
      {
        push(
          `Фора: ${minus.teamLabel ?? `К${minus.team}`} −${line} / ${plus.teamLabel ?? `К${plus.team}`} +${line}`,
          'maps_fora',
          minus,
          plus,
        );
...
export function analyze(match: Match): Analysis
{
  ...
  const divergence =
    shape?.p3FromHandicaps != null && shape?.p3FromTotals != null
...
{
  const fromHint = (fallbackHint || match.hint || '')
    .split(/\s+vs\s+/i)
