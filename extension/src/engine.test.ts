...
import {
  analyze, edgeOf, mergeSides, nameOf, parseOne, requiredP, seriesShape, stakeFor,
  type Match,
} from './engine';
...
describe('сетка исходов: две книги из трёх', () => {
  test('недостающее событие достраивается как остаток', async () => {
    ...
    near(s.pA, 0.280);       // 3DMAX
    near(s.pB, 0.720);       // FUT Esports
...
describe('реконструкция соперника по одной стороне', () => {
  ...
  test('но оценка разгрома по одной стороне занижена — за этим и нужен второй заход', async () => {
    ...
    const two = seriesShape(await both('bo3-two-fora-books.html', 'bo3-two-fora-books-side-b.html'))!;
...
describe('маржа по книгам', () => {
  ...
  test('центральная линия тотала раундов дешевле крайних', async () => {
    ...
    const byLine = (l: number) => books.find((b) => b.market === 'rounds_total' && b.label.endsWith(String(l)))!;
...
describe('расчёт ставки', () => {
  ...
  test('шкала размера соответствует README', () => {
    ...
    expect(stakeFor(0.10).units).toBe(1);
    expect(stakeFor(0.20).units).toBe(1.5);
    expect(stakeFor(0.30).units).toBe(2);
    expect(stakeFor(0.50).units).toBe(1);      // подозрительно высокий — режем, а не задираем
    expect(stakeFor(0.50).flag).toBe('🟡');
