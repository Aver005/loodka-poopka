...
(() => {
  ...
  function findTabs(root) {
    ...
    for (const scope of scopes) {
      ...
      for (const probe of [
        ...
        () => { const s = scope.querySelector('.team_select, [class*="team_select"]'); return s ? [...s.children] : []; },
        () => [...scope.querySelectorAll('.team_name, [class*="team_name"]')]
                .map((n) => n.closest('[class*="team_"]') ?? n.parentElement?.parentElement),
        () => [...scope.querySelectorAll('.team_stat_link, [class*="stat_link"]')]
                .map((n) => n.closest('[class*="team_"]') ?? n.parentElement?.parentElement),
      ]) {
  ...
  const sig = (root) => root ? [...root.querySelectorAll('.koef')].map((n) => n.textContent.trim()).join(',') : '';

  async function clickTab(tab, root) {
    const before = sig(root);
    const targets = [tab.querySelector('[class*="team_right"]'), tab.querySelector('img'), tab.firstElementChild, tab];
    for (const t of targets) {
      ...
      try { t.click?.(); } catch {}
      for (let i = 0; i < 16; i++) { // ~2 c на цель, дальше пробуем следующую
        await sleep(120);
        const now = sig(modal() ?? root);
        if (now && now !== before) { await sleep(400); return true; }
      }
    }
    return false;
  }
  ...
  function label(root) {
    ...
    const ev = root?.querySelector('.bet_event, [class*="bet_event"]')?.textContent.replace(/\s+/g, ' ').trim();
  ...
  function push(root, note) {
    const bag = load();
    const rec = { source: location.href, label: label(root), note, ts: Date.now(), html: root.outerHTML };
    const i = bag.findIndex((b) => b.label === rec.label && b.note === rec.note);
    if (i >= 0) { bag[i] = rec; store(bag); return { added: 0, updated: 1 }; }
    bag.push(rec);
    store(bag);
  ...
  const LP = {
    async grab() {
      const root = modal();
      if (!root) { log('Не нашёл коэффициенты на странице. Открой модалку и попробуй LP.debug()', '#c00'); return 0; }
      ...
      if (tabs.length !== 2) {
        ...
        console.log('   Исход и тотал карт восстановятся и так, а вот фора по картам будет неточной.');
      ...
      log(`${label(root)} — новых ${add}${upd ? `, обновлено ${upd}` : ''}, всего в буфере ${load().length}`);
      return add + upd;
    },

    async page() {
      const bag = load();
      bag.push({ source: location.href, label: 'FULL PAGE', note: 'аварийный режим', ts: Date.now(),
                 html: document.documentElement.outerHTML });
      store(bag);
      log(`Страница целиком сохранена (${(bag.at(-1).html.length / 1024 | 0)} КБ). Парсер сам найдёт модалку.`);
      return 1;
    },
    ...
    debug() {
      ...
      if (root) {
        ...
        console.log('  скелет:', [...root.children].map((c) => `${c.tagName}.${c.className}`).join('\n           '));
      }
      console.log('кандидаты во вкладки:', findTabs(root));
      console.log('узлы с team в классе:',
    ...
    count() {
      const b = load();
      console.table(b.map((x, i) => ({
        '#': i, матч: x.label, что: x.note, КБ: x.html.length / 1024 | 0,
    ...
    save(opts = {}) {
      ...
      else { localStorage.removeItem(KEY); log(`Скачано ${bag.length} записей, буфер очищен → файл в input/html/`); }
      return bag.length;
    },

    clear() { localStorage.removeItem(KEY); log('Сброшено.'); },
  };

  window.LP = LP;
  log('Готово. await LP.grab() на каждом матче → LP.save() в конце. Если что-то не так — LP.debug()');
})();
