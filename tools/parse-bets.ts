...
import {
  analyze, parseHtml, seriesShape, pairs, nameOf,
  TIER, TIER_MARK, EDGE_FLOOR, pct, oddsFmt, imp, requiredP,
  type MarketKey, type Match, type Offer, type TeamSlot,
...
function matchKey(m: Match): string {
  ...
  const mt = m.offers.filter((o) => o.market === 'maps_total')
    ...
    .map((o) => o.odds.toFixed(3)).join('/');
  if (mt) return `${m.tournament ?? '?'} | maps:${mt}`.toLowerCase();

  return (m.hint ?? m.sources[0] ?? '?').toLowerCase();
}

interface Merged extends Match { activeSlots: Set<TeamSlot | null> }
...
function merge(list: Match[]): Merged[] {
  ...
  const fp = (m: Match) => m.offers.filter((o) => o.market === 'maps_total')
    .sort((a, b) => (a.side ?? '').localeCompare(b.side ?? ''))
    .map((o) => o.odds.toFixed(3)).join('/');
...
function report(m: Merged): string {
  ...
  L.push(`Заходов: ${m.activeSlots.size} (кэфы стороны: **${A ? name(A) : '?'}**) · офферов: ${m.offers.length}`);
  L.push('');
  ...
  if (shape && A) {
    ...
    if (!shape.bSeriesActual && shape.bSeriesOdds) {
      L.push('');
      L.push(`> 🔮 **Реконструкция.** Кэф на ${name(B)} в серии не скачан, но выводится: ` +
    ...
    if (shape.mapCheck) {
      ...
      L.push('');
      L.push(`> ${d < 0.02 ? '✅' : '⚠️'} **Сверка через карту #1.** Книга даёт ${name(A)} ` +
    ...
    if (shape.p3FromHandicaps != null && shape.p3FromTotals != null) {
      ...
      L.push('>');
      L.push('> Маржа внутри каждой книги уже снята, так что это не её след, а расхождение ' +
  ...
  if (ps.length) {
    ...
    for (const p of sorted) {
      ...
      L.push(`| ${p.label} | ${TIER_MARK[TIER[p.market].tier]} | ${oddsFmt(p.a.odds)} → ${pct(p.a.fair)} | ` +
  ...
  for (const grade of ['A', 'B', 'C'] as const) {
    ...
    for (const [, list] of g) {
      for (const o of [...list].sort((x, y) => x.odds - y.odds)) {
        ...
        L.push(`| ${o.text || o.type}${tag} | **${oddsFmt(o.odds)}** | ${pct(imp(o.odds))} | ` +
...
await Bun.write(join(outDir, 'report.json'), JSON.stringify(
  matches.map((m) => ({ ...m, activeSlots: [...m.activeSlots], ...analyze(m), match: undefined })),
  null, 2));
